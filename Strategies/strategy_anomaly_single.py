...
# Instellingen voor de mappen
TARGET_FOLDER = "US500"
OUTPUT_CSV = os.path.join(TARGET_FOLDER, "us500_analyzed_data.csv")
OUTPUT_PLOT = os.path.join(TARGET_FOLDER, "us500_anomalies_chart.png")
...
def detect_us500_anomalies():
    print(f"🧠 ML Engine gestart voor asset: {TARGET_FOLDER}...")

    # 1️⃣ Automatisch het merged CSV-bestand opsporen in de map US500
    search_pattern = os.path.join(TARGET_FOLDER, "outputs_merged_*.csv")
    csv_files = sorted(glob.glob(search_pattern))

    if not csv_files:
        print(f"❌ Fout: Geen samengevoegd bestand gevonden (outputs_merged_*.csv) in map '{TARGET_FOLDER}'")
        return

    latest_merged_file = csv_files[-1]
    print(f"📂 Bestand ingeladen: {latest_merged_file}")
    ...
    # We berekenen de mid-prijs op basis van de beschikbare bid/ask kolommen
    df['close_mid'] = (df['close_bid'] + df['close_ask']) / 2
    df['US500_return'] = df['close_mid'].pct_change(periods=AGGREGATION_MINUTES)

    # We droppen de NaN-waardes die ontstaan door de pct_change
    ...
    # Selecteer de features voor de Isolation Forest (Rendement + Volume)
    ...
    for i in range(WINDOW_SIZE, len(df)):
        # Train het model op de afgelopen 240 minuten
        train_slice = market_features[i - WINDOW_SIZE : i]
        current_sample = market_features[i].reshape(1, -1)
        
        # We zetten contamination op 1% (de meest extreme 1% van de data is een anomalie)
    ...
    # Voeg de uitkomsten toe aan het dataframe
    df['anomaly_score'] = rolling_scores
    df['is_anomaly'] = rolling_anomalies

    # Sla de resultaten op in een nieuwe CSV binnen de US500 map
