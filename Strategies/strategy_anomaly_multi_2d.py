...
def run_multi_backtest():
    ...
    for i in range(len(df)):
        ...
        if position is not None:
            # 1. Check op Statistisch Herstel (Mean Reversion Convergence Target)
            converged = False
            ...
            # 2. Check op Tijdlimieten (EOD of Parachute Timeout)
            timeout = (i - entry_idx) >= MAX_DURATION
            forced_eod = curr_time > time(22, 0)

            # Als er een exit-trigger afgaat, wikkelen we de positie af
    ...
    # ---------------------------------------------------------------------
    # 📝 GEOPTIMALISEERDE LEDGER RAPPORTAGE MET MULTI-ASSET METRICS (.MD)
    # ---------------------------------------------------------------------
